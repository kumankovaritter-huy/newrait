        ...
        if missing_cols:
            ...
            with st.spinner("🪄 Анализируем данные..."):
                ...
                if df_problems.empty:
                    ...
                    # --- Группировка: строки одного артикула держим ВМЕСТЕ ---
                    # Ключ сортировки строки (приоритет -> отзывы -> сезон -> площадка ->
                    # рейтинг). Артикул в плане встаёт по своей ЛУЧШЕЙ строке (минимум
                    # ключа), а все его строки идут единым блоком следом.
                    df_problems['_ключ_строки'] = list(zip(
                        df_problems['Приоритет'],
                        df_problems['Сортировка_отзывы'],
                        df_problems['Сортировка_сезон'],
                        df_problems['Сортировка_площадка'],
                        df_problems['Рейтинг_число'].fillna(99),
                    ))
                    # ключ артикула = минимальный ключ среди его строк
                    sku_key = df_problems.groupby('СКУ_норм')['_ключ_строки'].transform('min')
                    df_problems['_ключ_артикула'] = sku_key

                    # сортируем: сначала артикулы по своему ключу, внутри — строки по своему
                    df_problems = df_problems.sort_values(
                        by=['_ключ_артикула', 'СКУ_норм', '_ключ_строки']
